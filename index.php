            ...
            if (isset($_SESSION['tasks'])) {
                ...
                foreach ($stmt->fetchAll() as $task) {
                    echo "<li>
                                ...
                                <button type='button' class='btn-edit' onclick='editar" . $task['id'] . "()'>Editar</button>
                                <script>
                                    function editar" . $task['id'] . "(){
                                        if (confirm('Confirma edição?')) {
                                            window.location='http://localhost:8100/task_edit.php?key=" . $task['id'] . "';
                                        }
                                        return false;
                                    }
                                </script>
